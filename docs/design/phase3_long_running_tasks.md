...
### 6.4 `deep_research` (added 2026-09-14 from the user's workflow)
Literature research by **citation snowballing**. Extends `research_report` (§6.1): same notes, reviewer, and report machinery, plus an iterative source-discovery loop whose tasks are generated by code each round.

**Seeds** (any combination):
- a folder of papers in the workspace (PDF, DOCX, MD, TXT)
- a pasted list of titles, DOIs, or arXiv links
- a search query: the agent searches open scholarly indexes (OpenAlex, arXiv, Semantic Scholar) and proposes a seed list; **the user approves the seed list** before reading starts

**Per paper**, one task each, reviewer on:
1. Get the text: local file, or open-access download (below).
2. **Section-by-section summaries**, each with key claims backed by verbatim quotes (`add_note`).
3. **Value of the paper:** contribution, methods, evidence strength, limitations, how it relates to the question.
4. **References:** extract the reference list into structured entries (title, authors, year, DOI/arXiv id when present), normalized for matching.
Results go to `papers/<paper-id>.md` plus the notes and citation tables.

**Citation loop (code-driven):**
1. After each round, aggregate references across all read papers into a citation graph: normalize, deduplicate (DOI, arXiv id, else fuzzy title+year), and count how many read papers cite each work.
2. Candidates for the next round are unread works ranked by citation count.
3. **Stop** when any limit is reached (decided 2026-09-14; the user may change them per job):
   - **convergence:** no unread work is cited by ≥ 3 read papers, or ≥ 15% of them, whichever is larger
   - **passes:** at most **4** citation rounds after the seed round
   - **sources:** at most **60** papers read
4. The foundational set is the most-cited works that were read. The report calls them out, and the reviewer checks that each is analyzed, not only listed.

**Getting cited papers:** look up each work in open indexes. Download a legal open-access PDF when one exists (arXiv, publisher OA, institutional repositories via OpenAlex/Unpaywall-style OA locations). Otherwise the paper's task parks with a request for the user to add the file, and other papers continue. Paywalled content is never fetched through unofficial sources. The approval model for these downloads is an open question (open_questions.md).

**Report:** plan the layout first (user gate): **abstract** at the front, introduction and scope, literature review organized by theme with the foundational works highlighted, synthesis (agreements, conflicts, gaps), and **conclusion and summary** at the end. Sections are drafted from the per-paper summaries and notes with citations, reviewed, then compiled to Markdown (functional) or DOCX (formal, decision 4). Includes a bibliography and a citation-graph appendix (most-cited works with counts, and papers that couldn't be obtained).
...
| 3b | Notes + FTS5 with verbatim-quote verification; document text extraction (txt/md/PDF/DOCX); reviewer pass (outputs and claims written to context); code-driven plan expansion and gates; `research_report` template; `deep_research` template (per-paper section summaries, reference extraction, citation graph, convergence/pass/source limits, open-access acquisition with user fallback, report layout gate, md/docx output) | Lake Veyra report meets the answer key's recall ≥ 80% with 100% verbatim quotes; a deep_research run on a small local paper set completes at least one citation round and stops by rule |
